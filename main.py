...
@app.get("/")
async def root():
    return {"message": "Hello World", "Sender":"Shutong Zhang"}


@app.get("/hello/{name}")
async def say_hello(name: str):
    return {"message": f"Awesome cloud developer sz3101 says hello {name}"}


@app.get("/hello_text/{name}")
async def say_hello_text(name: str):
    the_message = f"Awesome cloud developer sz3101 says Hello {name}"
